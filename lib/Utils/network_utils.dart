...
Future<Map<String, String>> buildHeaderTokens(bool buildAuthHeader) async {
  ...
  if (buildAuthHeader) {
    await JwtUtils.getJwtToken().then((token) {
...
Future<Response> buildHttpResponse(
  ...
  Map? request,
  bool biuldAuthHeader = false,
}) async {
  if (await isNetworkAvailable()) {
    Map<String, String> headers = await buildHeaderTokens(biuldAuthHeader);
    ...
    if (method == HttpMethod.POST ) {
      ...
      log('Request: $request');
      response = await patch(url, body: jsonEncode(request), headers: headers);
    } else {
...
Future handleResponse(Response response, [bool? avoidTokenError]) async {
  ...
  if (response.statusCode == 401) {
    if (!avoidTokenError.validate())
      LiveStream().emit('LIVESTREAM_TOKEN', true);
    throw 'Token Expired';
  }
